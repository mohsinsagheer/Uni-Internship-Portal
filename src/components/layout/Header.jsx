...
export default function Header({ onOpenAuth }) {
  const { currentUser, switchRole, campuses, selectedCampus, setSelectedCampus } = usePortal();
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-4 shrink-0 lg:ml-auto max-w-full">
            ...
            {currentUser?.role !== 'student' && (
              <select
                ...
                className="hidden md:block h-8 sm:h-9 max-w-[10rem] text-[11px] font-bold rounded-xl bg-white/10 text-white border border-white/25 px-2"
                title="Campus"
              >
                {campuses.map((c) => (
            ...
            <div className="relative">
              <button
                ...
                className="px-2.5 sm:px-3 h-8 sm:h-9 flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white font-bold rounded-xl border border-white/20 text-[11px] sm:text-xs"
              ...
              {roleMenuOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-slate-200 py-1 z-50">
                  {[
                    ...
                    { id: 'supervisor', label: 'Faculty Supervisor' },
                    { id: 'incharge', label: 'Internship Incharge' },
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs font-semibold hover:bg-slate-50 ${
                        currentUser?.role === item.id ? 'text-[#002147] bg-slate-50' : 'text-slate-700'
                      }`}
                    >
                      {item.label}
                    </button>
